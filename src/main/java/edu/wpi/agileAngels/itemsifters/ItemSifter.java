 ...
 *
 * <p>Filters are functions that take an item, and return true if its passes. See filter examples in
 * StringSearcher, ItemSifterTests, and the ISift javadoc.
 *
 * <p>Comparators are functions that take two items, and compare them. See java.util.Comparator. The
 * comparator is used to sort the items. If not provided, the items are not sorted.
 *
 * <p>Displays are functions that take a stream of items after they have been filtered and sorted.
 * It is up to you how you want to display the items, but see ListViewDisplay for a good example.
 *
 * <p>When the sifter updates, it runs all the items through its filters, sorts the filtered items
 * with its comparator (optional), and sends the items to all displays that are bound to it.
 *
 * <p>update(): filter -> sort -> display
 *
 * <p>Please email me at nllopez@wpi.edu to help me improve this so that it is more
