...
public class Parser {
    ...
    private String[] headers = {"NodeID", "xcoord", "ycoord",  "floor", "building", "nodeType", "longName", "shortName"};
    public HashMap<String, Location> locationData;
    /**
        ...
        try (Reader reader = Files.newBufferedReader(Paths.get(CSV_FILE_PATH));
            ...
            for (CSVRecord csvRecord : csvParser) {  //each row has a dictionary
                ...
                for (int i = 1; i < 9; i++) { //each item in the row
                    ((PreparedStatement) statement).setString(i, csvRecord.get(i - 1)); //to access the first value for table it starts at 1
